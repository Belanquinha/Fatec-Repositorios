...
- **UJ-1. Mariana posta o projeto que nasce aguardando aprovação.**
  ...
  - **Path:** clica "Entrar" → autentica no tenant CPS via MSAL → cai no papel aluno (e-mail `@aluno.cps.sp.gov.br`) → acessa "Novo projeto" → preenche dados (título, descrição curta, conteúdo EditorJS, link repositório, imagem capa, imagens extras, palavras-chave, ano publicado), insere o e-mail do professor responsável (com suporte a autocomplete de professores já cadastrados no tenant), vincula a instituição e registra os integrantes (nome, LinkedIn, papel) → publica.
...
- **UJ-3. Admin importa a lista oficial de instituições.**
  ...
  - **Path:** importa/edita a lista oficial das instituições CPS → dados ficam disponíveis para seleção.
...
- **Projeto** — Publicação com os seguintes atributos: `id` (UUID), `titulo` (string), `descricaoCurta` (string), `conteudoEditorJs` (text/json), `linkRepositorio` (string), `imagemCapaUrl` (string), `imagensExtras` (list de string), `palavrasChave` (list de string), `anoPublicado` (int), `estado` (Enum), `motivoRejeicao` (string), `emailProfessorResponsavel` (string), `instituicaoId` (UUID), `criadoEm` e `atualizadoEm` (LocalDateTime).
- **Integrante** — Membro da equipe do projeto (1 Projeto ── N Integrante) contendo: `id` (UUID), `projetoId` (UUID), `nome` (string), `linkLinkedin` (string) e `papelNoProjeto` (string).
- **Professor responsável** — Professor cujo e-mail é indicado no projeto (`emailProfessorResponsavel`). Front-end oferece autocomplete dos professores do tenant.
...
**Description:** O admin importa ou cura o catálogo com dados oficiais das instituições CPS.
...
O projeto exige e persiste `emailProfessorResponsavel` (autocomplete no front-end), `instituicaoId`, atributos completos do projeto e a lista de `Integrantes` (nome, linkLinkedin, papelNoProjeto).
...
O professor visualiza os projetos vinculados às instituições que selecionou.

#### FR-11: Aprovar/rejeitar projeto
O professor altera o estado para `APROVADO` ou `REJEITADO`. Quando rejeitado, `motivoRejeicao` é obrigatório.
...
2. Quem aprova: somente o professor com o e-mail responsável ou qualquer professor vinculado à instituição?